...
typedef struct
{
	EXTI_GPIO_Mapping_t 	EXTI_PIN;			// Specifies the EXTI GPIO mapping
												// This prameter must be a value of @ref EXTI_PINS_define

	uint8_t					Trigger_Case;		// Specifies triggering state (rising - falling - both)
												// This prameter must be a value of @ref TRIGGER_CASE_define

	uint8_t					IRQ_EN;				// Specifies triggering state (rising - falling - both)
												// This prameter must be a value of @ref EXTI_IRQ_define
